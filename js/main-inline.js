// Remainder of inline script (auto-migrated)
      ...
      // selectAllBtn.textContent = "Deselect All"; // Start as fully selected
      ...
      function normalizeRadarName(str) {
        return str
          .toLowerCase()
      ...
      // moved: theme util; see utils.js;
          ...
          for (const file of allFiles) {
            try {
              ...
              // Apply radar-based skipping logic BEFORE populating soundMeta
              const radarInfo = alr46Info[file];
              if (radarInfo) {
                if (
          ...
          populateSymbolFilter(soundMeta);
        } catch (error) {
      ...
      function populateSymbolFilter(sounds) {
        ...
        for (const sound of sounds) {
          ...
          const normalized = normalizeRadarName(
        ...
        selectAllWrapper.style.flexDirection = "column";
        selectAllWrapper.style.alignItems = "center";
        selectAllWrapper.style.justifyContent = "center";
        selectAllWrapper.style.width = "64px";
        selectAllWrapper.style.margin = "6px";
        ...
        // container.appendChild(selectAllWrapper); // ⬅️ as part of grid!

        Object.keys(symbolToImageMap).forEach((symbol) => {
          if (!usedSymbols.has(symbol)) return;

          const imageFile = symbolToImageMap[symbol];
          if (!imageFile) return;
      ...
      async function buildRadarSymbolMap() {
        if (window.radarSymbolMap) return;

        try {
          // Allow for custom emitter_id_data json.
          const [mainResponse, customResponse] = await Promise.all([
            fetch("jsons/emitter_id_data.json"),
            fetch("jsons/emitter_id_data_custom.json"),
          ]);

          const emitterDataMain = await mainResponse.json();
          const emitterDataCustom = await customResponse.json();

          // ✅ NEW: Process main data first, then custom data to give custom precedence
          window.radarSymbolMap = {};

          const addEntries = (entries, source, isCustom = false) => {
            for (const entry of entries) {
              const key = normalizeRadarName(entry.radar); // Normalize radar name for consistent dictionary key
              const newEntry = {
                symbol1: entry.symbol1,
                symbol2: entry.symbol2,
                source, // Source table this entry came from (e.g., ai_file_entries)
                repeat: entry.repeat, // Repeat value used to determine priority (can be 1, a number, or false)
                warning: entry.warning, // ✅ NEW: Capture warning field
                warning2: entry.warning2, // ✅ NEW: Capture warning2 field
                isCustom, // ✅ NEW: Track if this is from custom file
              };

              if (!window.radarSymbolMap[key]) {
                // First time encountering this radar — initialize entry list with this source
                window.radarSymbolMap[key] = [newEntry];
              } else {
                const existingEntries = window.radarSymbolMap[key];

                // Look for an existing entry from the same source (e.g., new_sea_file)
                const existingIdx = existingEntries.findIndex((e) => e.source === source);

                if (existingIdx === -1) {
                  // No entry from this source yet — safe to add
                  existingEntries.push(newEntry);
                } else {
                  // Existing entry from this source found — decide whether to replace based on priority
                  ...
                  // 1. Custom data ALWAYS takes precedence over main data
                  // 2. repeat === 1 always takes precedence (within same data source)
                  // 3. A numeric repeat beats `false` (within same data source)
                  // 4. If both are numbers, the lower one has higher priority (within same data source)
                  let shouldReplace = false;

                  if (isCustom && !existing.isCustom) {
                    // Custom data always replaces main data
                    shouldReplace = true;
                  } else if (!isCustom && existing.isCustom) {
                    // Never replace custom data with main data
                    shouldReplace = false;
                  } else {
                    // Same data source type, use original priority logic
                    if (currentRepeat === 1) {
                      shouldReplace = true;
                  ...
                  // Replace the existing entry for this source only if the new one has higher priority
                  if (shouldReplace) {
          ...
          console.log(
            ...
            Object.keys(window.radarSymbolMap).length,
            "entries (custom data takes precedence)"
          );

          // Debug: Log some entries to verify custom precedence
          Object.entries(window.radarSymbolMap)
            .slice(0, 5)
            .forEach(([key, entries]) => {
              entries.forEach((entry) => {
                const dataSource = entry.isCustom ? "CUSTOM" : "MAIN";
                console.log(
                  `🔍 ${key}: ${dataSource}, repeat=${entry.repeat}, symbol1=${entry.symbol1}`
                );
              });
            });
        } catch (e) {
        ...
        for (const groupName of Object.keys(grouped).sort()) {
          ...
          for (const sound of sortedSounds) {
            ...
            if (radarInfo) {
              ...
              // Auto hide DEPRECATED LOCATION as it effects "Ungrouped" still appearing
              // if (
              //   // If wav is SEARCH and radar type is TRACK ONLY and PRF between both modes are same then hide
              //   fullPath.includes("SEARCH") &&
              //   radarInfo.type == "TRACK_ONLY" &&
              //   radarInfo.prf_search == radarInfo.prf_track
              // ) {
              //   console.log(`🔒 Hidden (SEARCH wav with TRACK_ONLY radar, same PRF): ${fullPath}`);
              //   continue;
              // } else if (
              //   // If wav is TRACK and radar type is SEARCH then hide
              //   fullPath.includes("TRACK") &&
              //   radarInfo.type == "SEARCH_ONLY"
              //   // radarInfo.prf_search == radarInfo.prf_track
              // ) {
              //   console.log(`🔒 Hidden (TRACK wav with SEARCH_ONLY radar): ${fullPath}`);
              //   continue;
              // } else if (
              //   fullPath.includes("TRACK") &&
              //   radarInfo.type == "SEARCH_AND_TRACK" &&
              ...
              // ) {
              //   console.log(
              //     `🔒 Hidden (TRACK wav with same PRF as SEARCH wav with SEARCH_AND_TRACK): ${fullPath}`
              //   );
              //   continue;
              // }
            ...
            const normalizedOptions = [
              normalizeRadarName(strippedFile + ".wav"),
              normalizeRadarName(strippedFile),
              normalizeRadarName(sound.name),
              normalizeRadarName(sound.description),
            ];
            ...
            // Helper function to get unknown symbol based on frequency
            const getUnknownSymbolFromFrequency = (freqGHz) => {
            ...
            // Helper function to create symbol with label
            const createSymbolWithLabel = (symbolValue, symbolNumber, isUnknown = false) => {
              const imageFileName = symbolToImageMap[symbolValue];
              if (!imageFileName) return null;

              // ✅ NEW: Check if this is a known emitter using an unknown symbol
              ...
              // Debug logging
              if (isKnownUnknownSymbol) {
                console.log(`🔍 Known emitter using unknown symbol: ${symbolValue}`);
              }
              ...
              label.style.color =
                isUnknown || isKnownUnknownSymbol ? "#ff6b35" : "var(--highlight)"; // Orange for unknown
              ...
              img.onerror = () => {
                console.warn(
                  `❌ Failed to load ${
                    isUnknown || isKnownUnknownSymbol ? "unknown" : "symbol" + symbolNumber
                  } image:`,
                  img.src
                );
            ...
            const createWarningDisplay = (warnings) => {
            ...
            // Replace the existing radar symbol display section (around line 800-950) with this updated version:
            if (radarEntry) {
              // ✅ Loop through each radarEntry (could be multiple if radar exists in multiple tables)
              for (const radarEntryVariant of Array.isArray(radarEntry)
                ? radarEntry
                : [radarEntry]) {
                const { symbol1, symbol2, source, warning, warning2 } = radarEntryVariant;

                // ✅ Check if this is a manually assigned unknown symbol
                ...
                // ✅ Render first RWR symbol (if defined)
                const symbol1Element = createSymbolWithLabel(symbol1, 1, isManualUnknownSymbol);
                if (symbol1Element) {
                  symbolContainer.appendChild(symbol1Element);
                }

                // ✅ Render second RWR symbol (only if it's different from symbol1)
                if (symbol2 && symbol2 !== symbol1) {
                  ...
                  const symbol2Element = createSymbolWithLabel(symbol2, 2, isSymbol2Unknown);
                  if (symbol2Element) {
                ...
                // ✅ Handle different display modes based on symbol type
                if (isManualUnknownSymbol) {
                  // For manually assigned unknown symbols, show frequency band info like auto-detected unknowns
                  const radarInfo = alr46Info[fullPath];
                  ...
                  // Add warning for manually assigned unknown symbols
                  // const warningText = document.createElement("div");
                  // warningText.innerHTML = `<b style="color: #ff6b35;">⚠️ WARNING:</b> This radar uses an unknown symbol.<br>Symbol assigned based on frequency band.`;
                  // warningText.style.fontSize = "0.85em";
                  ...
                  // For normal symbols, show source table info
                  const friendlySourceName = sourceNameMap[source] || source;
                ...
                // ✅ Display warnings if they exist (for all symbol types)
                const warnings = [];
                if (warning && warning.trim()) warnings.push(warning.trim());
                if (warning2 && warning2.trim()) warnings.push(warning2.trim());

                const warningDisplay = createWarningDisplay(warnings);
                if (warningDisplay) {
                  extraInfo.appendChild(warningDisplay);
                }
              }
            } else {
              // Handle unknown radars - check if we have frequency data
              const radarInfo = alr46Info[fullPath];
              ...
              if (radarInfo && radarInfo.band != null) {
                const freqGHz = Number(radarInfo.band);
                unknownSymbol = getUnknownSymbolFromFrequency(freqGHz);
              ...
              if (unknownSymbol) {
                ...
                const unknownElement = createSymbolWithLabel(unknownSymbol, null, true);
                if (unknownElement) {
                ...
                // Warning message for unknown radar (existing code)
                const warningText = document.createElement("div");
      ...
      function showAllRadarInfo() {
        document.querySelectorAll(".extra-info").forEach((info) => {
          info.style.display = "block";
        });
      }
      function hideAllRadarInfo() {
        document.querySelectorAll(".extra-info").forEach((info) => {
          info.style.display = "none";
        });
      }
      // Dropdown toggle setup (only do this once!)
      ...
      toggleBtn.addEventListener("click", (e) => {
        e.stopPropagation(); // prevents outer click from immediately closing it
